        ...
        <div className="flex justify-around items-center h-16">
          {navItems.map((item) => (
            <NavLink
              ...
              key={item.path}
              className={({ isActive }) =>
                `relative flex flex-col items-center justify-center px-3 py-2 rounded-xl transition-all duration-300 min-w-[60px] ${
                  isActive ? "text-blue-600 transform scale-105" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"
                }`
              }
            >
              {({ isActive }) => (
                <>
                  ...
                  {/* Active background */}
                  {isActive && <div className="absolute inset-0 bg-blue-50/50 rounded-xl -z-10"></div>}
                </>
              )}
            </NavLink>
          ))}
